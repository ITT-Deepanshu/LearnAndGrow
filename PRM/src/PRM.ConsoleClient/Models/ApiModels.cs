...
public sealed record LoginResponse(string AccessToken, string RefreshToken, bool ForcePasswordChange, string Role, string FullName);
public sealed record MeResponse(long Id, string Username, string Email, string FullName, string Role, bool ForcePasswordChange);

public sealed record CreateUserRequest(string FullName, string Email, string Username, string TemporaryPassword, string Role);
public sealed record UserResponse(long Id, string Username, string Email, string FullName, string Role, bool IsActive, bool ForcePasswordChange);
...
public sealed record EmployeeListItem(
    long Id, long UserId, string FullName, string Department, string Designation,
    string Status, long? ManagerId, string ManagerName, int SkillCount);
public sealed record EmployeeSkill(long Id, string Name, string Category, string Proficiency);
public sealed record EmployeeDetail(
    long Id, long UserId, string Username, string Email, string FullName,
    string Department, string Designation, string Status, long? ManagerId,
    string ManagerName, DateTime? JoinedAt, IReadOnlyList<EmployeeSkill> Skills);

public sealed record CreateProjectRequest(
    string Name, string Description, DateOnly StartDate, DateOnly EndDate,
    int Status, long ManagerId, int TotalStoryPoints);
...
public sealed record ProjectDetail(
    long Id, string Name, string Description, DateOnly StartDate, DateOnly EndDate,
    string Status, long ManagerId, string ManagerName, int TotalStoryPoints, string Health,
    string? HealthReason, IReadOnlyList<Milestone> Milestones, IReadOnlyList<AllocationSummary> Allocations);
...
public sealed record AllocationSummary(long Id, long EmployeeId, string EmployeeName, decimal UtilisationPercentage, DateOnly FromDate, DateOnly ToDate);
public sealed record ProjectHealth(long ProjectId, string Status, string DisplayLabel, IReadOnlyList<string> RiskFlags);

public sealed record CreateAllocationRequest(long ProjectId, long EmployeeId, decimal UtilisationPercentage, DateOnly FromDate, DateOnly ToDate);
public sealed record Allocation(
    long Id, long EmployeeId, string EmployeeName, long ProjectId, string ProjectName,
    decimal UtilisationPercentage, DateOnly FromDate, DateOnly ToDate, DateOnly? EndedAt);
public sealed record AllocationListItem(
    long Id, long EmployeeId, string EmployeeName, long ProjectId, string ProjectName,
    decimal UtilisationPercentage, DateOnly FromDate, DateOnly ToDate);
...
public sealed record Timesheet(long Id, DateOnly WeekStart, string Status, DateTime? SubmittedAt, decimal TotalHours, IReadOnlyList<TimesheetEntry> Entries);
...
public sealed record ResourceDashboard(
    DateOnly AsOfDate, DashboardCounts Counts, IReadOnlyList<BenchEmployeeRow> OnBench,
    IReadOnlyList<ActiveEmployeeRow> PartiallyAllocated, IReadOnlyList<ActiveEmployeeRow> FullyAllocated,
    IReadOnlyList<SkillCategorySummary> SkillsSummary, IReadOnlyList<EmployeeDrillDown> DrillDown);
...
public sealed record ActiveEmployeeRow(long Id, string FullName, string Department, decimal UtilisationPercentage, decimal AvailabilityPercentage, IReadOnlyList<string> Skills);
...
public sealed record EmployeeDrillDown(
    long Id, string FullName, string Department, string Designation, string Status,
    decimal UtilisationPercentage, decimal AvailabilityPercentage, IReadOnlyList<string> ProfileSkills,
    IReadOnlyList<EmployeeAllocationRow> ActiveAllocations, IReadOnlyList<string> RecentActivityTags);
...
public sealed record RankedCandidate(long EmployeeId, string Name, string Reason, decimal? SuggestedUtilisation);
...
public sealed record StoredTokens(string? AccessToken, string? RefreshToken, long? EmployeeId);
